...
class Architect:
  def __init__(self, master, employeeId, payment_status, rukhsah_status):
    ...
    ttk.Button(self.frame_content, text = 'Tasks', command=self.view_all_tasks).grid(row = 0, column= 1,   padx= 10, pady=20)
    ttk.Button(self.frame_content, text = 'Upload Architectural Plan', command=self.upload_arch_plan).grid(row = 1, column= 1,   padx= 10, pady=20)
    ...
    ttk.Button(self.frame_content, text = 'Make a Task', command=self.insert_new_task).grid(row = 4, column= 1,   padx= 10, pady=20)
  ...
  def insert_new_task(self):
    ...
    Task(task_win,self.employeeId, self.customerName, self.customerId)
