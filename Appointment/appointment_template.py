...
class Appointment:
  def __init__(self, master, employeeId, customerName, customerId):
    ...
    self.entry_customerId.grid(row = 1, column = 0, padx = 5)

    ttk.Label(self.frame_content, text = 'Phone No:').grid(row = 0, column = 1)
    self.entry_customerName = ttk.Entry(self.frame_content, width = 24, font = ('Arial', 14))   
    self.entry_customerName.grid(row = 1, column = 1, padx = 5)

    ttk.Label(self.frame_content, text = 'Date:').grid(row = 0, column = 2)
    self.entry_app_date = ttk.Entry(self.frame_content, width = 24, font = ('Arial', 14))   
    self.entry_app_date.grid(row = 1, column = 2, padx = 5)

    ttk.Label(self.frame_content, text = 'Employee Id: ').grid(row = 2, column = 0, padx = 5, pady = 5,sticky='sw')

     # -----------Select from Combobox-------------------
    self.employee_Id = StringVar()
    self.employee_Id_cb = ttk.Combobox(self.frame_content, textvariable=self.employee_Id)
    self.employee_Id_cb.grid(row=2, column=1, padx=5)
    # --------Add values to the combobox-----
    self.employee_Id_cb.config(values = ('Architect', 'Structural', 'Inspector'))
    # self.payment_type.get()  #to get the chosen value
  

    # self.cal = Calendar(self.frame_content, showweeknumbers=False, foreground='black', background='blue', font=(14),selectedmode="day", year=2021, month=11, day=22)
    # self.cal.grid(row = 2, column=0, pady=20, padx=20)
    # self.cal.get_date()
  ...
  def submit(self):    
    ...
    save_customer_appointments(self.employee_Id.get(), self.entry_customerId.get(), self.entry_customerName.get(), self.entry_app_date.get())
